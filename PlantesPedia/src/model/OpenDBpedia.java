...
public class OpenDBpedia {
    ...
    String req = "Select distinct ?ville Where{ ?ville <http://dbpedia.org/ontology/country> <http://dbpedia.org/resource/France>. ?ville rdf:type <http://fr.dbpedia.org/ontology/Settlement>";
...
public static String Region(String region){
    return ". ?ville <HTTP://fr.dbpedia.org/ontology/region> <HTTP://fr.dbpedia.org/resource/"+region+"/>";
}

public static String Departement(String departement){
    return ". ?ville <HTTP://fr.dbpedia.org/ontology/department> <HTTP://fr.dbpedia.org/resource/"+departement+"/>";
}
public static String Code_postal(int CP){
    return ". ?ville <HTTP://fr.dbpedia.org/ontology/postalCode> ? CP . FILTER(?CP ="+Integer.toString(CP)+")";
}

public static String Population(int min, int max){
    return ". ?ville <HTTP://fr.dbpedia.org/ontology/populationTotal> ? POP . FILTER(?POP >"+Integer.toString(min)+") .FILTER(?POP <"+
            Integer.toString(max)+")";
}

public static String Superficie(int min, int max){
    return ". ?ville <HTTP://fr.dbpedia.org/ontology/area> ? SUP . FILTER(?SUP >"+Integer.toString(min)+") .FILTER(?SUP <"+
            Integer.toString(max)+")";
}
